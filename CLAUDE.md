...
### 表現
- **用途を限定しない。汎用的な表現にする**（×「提案書に使うとき」→ ○ 用途を特定しない表現）。**例外**：「成果物を作る」系セクション（1-7～1-11、2-3・2-4）は、受講者が全員コンサルタントであるため戦略・マーケティングコンサル／マーケティングリサーチの具体例（市場規模算出・競合比較・ポジショニングマップ等）を優先してよい
